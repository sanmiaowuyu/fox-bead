...
> 维护人：余莎莎 ｜ 最后更新：2026-08-10（#18 去背景渐变鲁棒 / #19 小程序UI / #20 手绘改色 / #21 主预览平移 / 图片处理模块 / 视觉升级三态主题 / 豆仓库存模块 / 竞品分析文档 / 修复下载·预览放大·调色注释·默认深色 / v141 图片处理重构·自动抠图多主体·去笔刷·删提亮 / v142 羽化强度可调·灯箱直转像素·主体留边·批量对照长图·分块多板导出 / v143 补货清单导出·分块多板PDF·小程序发版就绪·保护笔刷·设置持久化·多色板切换 / v144 分块多板补跨板对齐标记·CC复核意见②核查no-op / iframe预览下载修复 / 下载修复v2-clipboard逃生通道）
...
| 2026-08-11 | **下载修复 v2（沙箱 clipboard 逃生通道·不 bump）**：用户反馈昨日 `下载图片`/`新窗口打开` 在 CloudStudio 预览里仍无效——根因是预览 iframe 为 **sandboxed 且无 `allow-downloads`/`allow-popups` 令牌**，`<a download>` 与 `window.open` 被静默拦截，右键又被宿主 UI 吞掉。新方案：保存对话框改以 **「复制图片」为主按钮**，调用 `navigator.clipboard.write`（`ClipboardItem` 写 image/png），在同源沙箱内可用，用户复制到剪贴板后去微信/画图/备忘录 `Ctrl+V` 即可存图；失败兜底复制 dataURL 文本（粘到浏览器地址栏下载）。新增 `dataURLToBlob`/`copyImageToClipboard` 辅助；`下载图片`/`新窗口打开` 保留为次按钮；PDF 走同名对话框但隐藏「复制图片」。仍为同一「无法下载」bug 的延续修复，不 bump | 用户「昨天还是无法下载图片」 | src/web/exporter.js | ✅ |
