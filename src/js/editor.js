...
/* ---------- 放大预览：生成高清图案区 canvas，支持滚轮缩放与拖拽 ---------- */
function buildPatternCanvas(cellSize = 8) {
  const N = state.N;
  const dr = state.displayRect;
  const M = dr ? dr.M : N;
  const cell = Math.max(4, Math.round(cellSize));
  const showCoords = state.showCoords;
  const marginSide = showCoords ? Math.round(cell * 2.5) : 0;
  const marginLeft = marginSide, marginBottom = marginSide;
  const px = cell * M;
  const W = px + marginLeft;
  const H = px + marginBottom;
  ...
  if (state.view === 'original' && state.sourceImage) {
    ...
    const dx = ox + Math.floor((px - dw) / 2);
    const dy = oy + Math.floor((px - dh) / 2);
    if (state.mirror) {
      c.save();
      c.translate(dx + dw, dy);
      c.scale(-1, 1);
      c.drawImage(state.sourceImage, cr.sx, cr.sy, cr.sw, cr.sh, 0, 0, dw, dh);
      c.restore();
    } else {
      c.drawImage(state.sourceImage, cr.sx, cr.sy, cr.sw, cr.sh, dx, dy, dw, dh);
    }
  } else if (state.grid) {
    // v100: 绘制豆子 M×M，映射到 grid 源坐标（和导出一致）
    for (let gy = 0; gy < M; gy++) {
      for (let gx = 0; gx < M; gx++) {
        ...
        if (id == null) continue;
        const dx = state.mirror ? (M - 1 - gx) : gx;
        c.fillStyle = PALETTE_BY_ID[id].hex;
        c.fillRect(ox + dx * cell, oy + gy * cell, cell, cell);
      }
    }
  }

  // 豆子间隔线（始终显示，极淡）：让白色/浅色豆在白底上也有边界
  const borderColor = state.bgMode === 'black' ? 'rgba(255,255,255,0.25)' : 'rgba(0,0,0,0.25)';
  drawBeadBorders(c, ox, oy, M, M, cell, borderColor);

  // 网格线：每 1 格虚线 + 每 10 格实线
  if (state.showGrid) {
    const interval = 10;
    for (let i = 0; i <= M; i++) {
      ...
      const p = ox + i * cell;
      if (major) {
        c.strokeStyle = 'rgba(30,30,30,0.65)';
        c.lineWidth = Math.max(2, Math.round(cell * 0.12));
        c.setLineDash([]);
      } else {
        c.strokeStyle = 'rgba(150,150,150,0.35)';
        c.lineWidth = Math.max(1, Math.round(cell * 0.05));
        const dash = Math.max(2, Math.round(cell * 0.4));
        c.setLineDash([dash, dash]);
      }
      c.beginPath(); c.moveTo(p, oy); c.lineTo(p, oy + M * cell); c.stroke();
      c.beginPath(); c.moveTo(ox, oy + i * cell); c.lineTo(ox + M * cell, oy + i * cell); c.stroke();
    }
    c.setLineDash([]);
  }
  ...
  if (showCoords) {
    const interval = 10;
    const fs = Math.max(10, Math.round(cell * 0.65));
    c.fillStyle = '#6B6675';
    c.font = `${fs}px monospace`;
    c.textAlign = 'right'; c.textBaseline = 'middle';
    for (let i = 0; i < M; i += interval) {
      c.fillText(i.toString(), marginLeft - 4, oy + i * cell + cell / 2);
    }
    c.textAlign = 'center'; c.textBaseline = 'top';
    for (let i = 0; i < M; i += interval) {
      c.fillText(i.toString(), ox + i * cell + cell / 2, oy + M * cell + 4);
    }
  }
...
function openPreview() {
  if (!state.grid && !(state.view === 'original' && state.sourceImage)) {
    alert('请先上传图片或载入示例');
    return;
  }
  // 根据网格尺寸选择清晰度：网格越大每格越小，但总量不超过合理上限
  const dr = state.displayRect;
  const M = dr ? dr.M : state.N;
  const availW = Math.min(window.innerWidth * 0.9, 2048);
  const baseCell = Math.max(6, Math.floor(availW / M));
  preview.cell = Math.min(baseCell, 14); // 上限 14，保证性能与清晰平衡
  preview.pattern = buildPatternCanvas(preview.cell);
  preview.zoom = 1;
  preview.pan = { x: 0, y: 0 };
...
function closePreview() { $('preview-backdrop').hidden = true; }
function fitPreview() {
  const pattern = preview.pattern;
  if (!pattern) return;
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  const margin = 60;
  const scaleX = (vw - margin) / pattern.width;
  const scaleY = (vh - margin) / pattern.height;
  preview.zoom = Math.min(scaleX, scaleY, 1);
  preview.pan = { x: 0, y: 0 };
}
function renderPreview() {
  const pattern = preview.pattern;
  if (!pattern || !previewCtx) return;
  const vw = window.innerWidth;
  const vh = window.innerHeight;
  ...
  // 让图案居中 + 平移
  const drawW = pattern.width * preview.zoom;
  const drawH = pattern.height * preview.zoom;
  const baseX = (vw - drawW) / 2 + preview.pan.x;
  const baseY = (vh - drawH) / 2 + preview.pan.y;
  previewCtx.imageSmoothingEnabled = preview.zoom < 1;
  previewCtx.drawImage(pattern, baseX, baseY, drawW, drawH);
}
function previewZoomBy(delta, cx, cy) {
  const old = preview.zoom;
  const next = Math.min(preview.maxZoom, Math.max(preview.minZoom, old * delta));
  if (next === old) return;
  // 以鼠标位置为中心缩放
  const vw = window.innerWidth, vh = window.innerHeight;
  const patW = preview.pattern.width, patH = preview.pattern.height;
  // 鼠标相对图案中心的位置
  const centerX = vw / 2 + preview.pan.x;
  const centerY = vh / 2 + preview.pan.y;
  const dx = cx - centerX;
  const dy = cy - centerY;
  // 缩放后平移补偿，使鼠标下图案点不动
  preview.pan.x += dx * (1 - next / old);
  preview.pan.y += dy * (1 - next / old);
  preview.zoom = next;
  renderPreview();
}
function bindPreviewEvents() {
  ...
  backdrop.addEventListener('wheel', e => {
    ...
    previewZoomBy(delta, e.clientX, e.clientY);
  }, { passive: false });
  // 拖拽平移
  backdrop.addEventListener('mousedown', e => {
    if (e.target === $('preview-close')) return;
    preview.dragging = true;
    preview.last = { x: e.clientX, y: e.clientY };
    backdrop.classList.add('dragging');
  });
  window.addEventListener('mousemove', e => {
    if (!preview.dragging) return;
    const dx = e.clientX - preview.last.x;
    const dy = e.clientY - preview.last.y;
    preview.pan.x += dx;
    preview.pan.y += dy;
    preview.last = { x: e.clientX, y: e.clientY };
    renderPreview();
  ...
  // 双击关闭
  backdrop.addEventListener('dblclick', closePreview);
  // 关闭按钮
  const previewCloseBtn = $('preview-close');
  if (previewCloseBtn) previewCloseBtn.addEventListener('click', closePreview);
  // 窗口大小变化时重绘
  window.addEventListener('resize', () => {
    if (!$('preview-backdrop').hidden) renderPreview();
  });
}
